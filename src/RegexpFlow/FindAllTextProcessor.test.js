	...
	describe("import/export object", function () {
		...
		describe("initializeFromObject()", function () {
			...
			it("copyPropertiesFrom copies specified properties", function () {
				findAllTextProcessor.initializeFromObject(sourceObject);

				expect(findAllTextProcessor.searchString).toEqual('[a-z]');
				expect(findAllTextProcessor.searchFlagCaseInsensitive).toEqual(true);
				expect(findAllTextProcessor.isEnabled).toEqual(true);
				expect(findAllTextProcessor.description).toEqual('foobar');
			});
		...
		describe("getExportObject()", function () {
			...
			beforeEach(function () {
				findAllTextProcessor.searchString = 'foo';
				findAllTextProcessor.searchFlagCaseInsensitive = true;
				findAllTextProcessor.isEnabled = true;
				findAllTextProcessor.description = 'foobar';
			});

			it("extractPropertiesToObject exports empty object if no properties are specified", function () {
				var exportedObject = findAllTextProcessor.getExportObject();
				expect(exportedObject).toEqual({
					typeName: 'FindAllTextProcessor',
					searchString: 'foo',
					searchFlagCaseInsensitive: true,
					isEnabled: true,
					description: 'foobar'
