	...
	describe("import/export object", function () {
		...
		describe("initializeFromObject()", function () {
			...
			it("copyPropertiesFrom copies specified properties", function () {
				replaceTextProcessor.initializeFromObject(sourceObject);

				expect(replaceTextProcessor.searchString).toEqual('[a-z]');
				expect(replaceTextProcessor.replaceString).toEqual('--$1--');
				expect(replaceTextProcessor.searchFlagGlobal).toEqual(true);
				expect(replaceTextProcessor.searchFlagMultiline).toEqual(true);
				expect(replaceTextProcessor.searchFlagCaseInsensitive).toEqual(true);
				expect(replaceTextProcessor.isEnabled).toEqual(true);
				expect(replaceTextProcessor.description).toEqual('foobar');
			});
		});
		...
		describe("getExportObject()", function () {
			...
			beforeEach(function () {
				replaceTextProcessor.searchString = 'foo';
				replaceTextProcessor.replaceString = '--$1--';
				replaceTextProcessor.searchFlagGlobal = true;
				replaceTextProcessor.searchFlagMultiline = true;
				replaceTextProcessor.searchFlagCaseInsensitive = true;
				replaceTextProcessor.isEnabled = true;
				replaceTextProcessor.description = 'foobar';
			});

			it("extractPropertiesToObject exports empty object if no properties are specified", function () {
				var exportedObject = replaceTextProcessor.getExportObject();
				expect(exportedObject).toEqual({
					typeName: 'ReplaceTextProcessor',
					searchString: 'foo',
					replaceString: '--$1--',
					searchFlagGlobal: true,
					searchFlagMultiline: true,
