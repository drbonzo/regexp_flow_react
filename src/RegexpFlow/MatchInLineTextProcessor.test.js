	...
	describe("import/export object", function () {
		...
		describe("initializeFromObject()", function () {
			...
			it("copyPropertiesFrom copies specified properties", function () {
				matchInLineTextProcessor.initializeFromObject(sourceObject);

				expect(matchInLineTextProcessor.searchString).toEqual('[a-z]');
				expect(matchInLineTextProcessor.searchFlagCaseInsensitive).toEqual(true);
				expect(matchInLineTextProcessor.isEnabled).toEqual(true);
				expect(matchInLineTextProcessor.description).toEqual('foobar');
			});
		...
		describe("getExportObject()", function () {
			...
			beforeEach(function () {
				matchInLineTextProcessor.searchString = 'foo';
				matchInLineTextProcessor.searchFlagCaseInsensitive = true;
				matchInLineTextProcessor.isEnabled = true;
				matchInLineTextProcessor.description = 'foobar';
			});

			it("extractPropertiesToObject exports empty object if no properties are specified", function () {
				var exportedObject = matchInLineTextProcessor.getExportObject();
				expect(exportedObject).toEqual({
					typeName: 'MatchInLineTextProcessor',
					searchString: 'foo',
					searchFlagCaseInsensitive: true,
					isEnabled: true,
					description: 'foobar'
