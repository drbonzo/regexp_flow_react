...
class ReplaceFilter extends Filter {
    ...
    processText(filterConfig, inputText) {
        ...
        try {
            ...
            let searchRegexp = this.buildRegExp(filterConfig.searchString, filterConfig.caseInsensitive, filterConfig.global, filterConfig.multiline);
            let matches = inputText.match(searchRegexp);
            filterConfig.replacementsCount = (matches ? matches.length : 0); // matches is null when no match is found

            let replacement = filterConfig.replaceString;
