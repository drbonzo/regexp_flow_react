	...
	describe("import/export object", function () {
		...
		describe("initializeFromObject()", function () {
			...
			it("copyPropertiesFrom copies specified properties", function () {
				matchLinesTextProcessor.initializeFromObject(sourceObject);

				expect(matchLinesTextProcessor.searchString).toEqual('[a-z]');
				expect(matchLinesTextProcessor.searchFlagCaseInsensitive).toEqual(true);
				expect(matchLinesTextProcessor.flagInvertMatch).toEqual(true);
				expect(matchLinesTextProcessor.isEnabled).toEqual(true);
				expect(matchLinesTextProcessor.description).toEqual('foobar');
			});
		...
		describe("getExportObject()", function () {
			...
			beforeEach(function () {
				matchLinesTextProcessor.searchString = 'foo';
				matchLinesTextProcessor.searchFlagCaseInsensitive = true;
				matchLinesTextProcessor.flagInvertMatch = true;
				matchLinesTextProcessor.isEnabled = true;
				matchLinesTextProcessor.description = 'foobar';
			});

			it("extractPropertiesToObject exports empty object if no properties are specified", function () {
				var exportedObject = matchLinesTextProcessor.getExportObject();
				expect(exportedObject).toEqual({
					searchString: 'foo',
					searchFlagCaseInsensitive: true,
					flagInvertMatch: true,
					isEnabled: true,
					typeName: 'MatchLinesTextProcessor',
					description: 'foobar'
				});
			});
		});
